...
#if defined(CL_DEVICE_TOPOLOGY_AMD) && !defined(_WIN32)
...
int main(int argc, char * * argv) {
	...
	try {
		...
		argp.addSwitch('p', "prefix", strPrefix);
		argp.addSwitch('x', "suffix", strSuffix);
		...
		if (!strPrefix.empty() || !strSuffix.empty()) {
			...
			std::string pattern = strPrefix;
			for (size_t i = 0; i < 40 - total; ++i) pattern += 'X';
			pattern += strSuffix;
			strModeMatching = pattern;
		}
		...
		// Auto-generate keypair if -z not provided
		extern bool g_bAutoKey;
		extern mp_number g_seedPrivKey;
		if (strPublicKey.empty()) {
			g_bAutoKey = true;
			std::cout << "Generating seed keypair..." << std::flush;
			KeyGen::generatePrivateKey(g_seedPrivKey);
			point pubPoint;
			KeyGen::computePublicKey(g_seedPrivKey, pubPoint);
			strPublicKey = KeyGen::publicKeyToHex(pubPoint);
		...
		if (vDeviceBinary.size() == vDevices.size()) {
			...
			const std::string strKeccak = embedded_keccak_cl;
			const std::string strVanity = embedded_profanity_cl;
		...
		// Auto-quit when prefix+suffix fully matched
		// Score is per byte (2 hex chars = 1 score point), round up for odd lengths
		cl_uchar scoreQuit = 0;
		if (!strPrefix.empty() || !strSuffix.empty()) {
			scoreQuit = (cl_uchar)((strPrefix.size() + 1) / 2 + (strSuffix.size() + 1) / 2);
		}
		Dispatcher d(clContext, clProgram, mode, worksizeMax == 0 ? inverseSize * inverseMultiple : worksizeMax, inverseSize, inverseMultiple, scoreQuit, strPublicKey);
		for (auto & i : vDevices) {
