...
static void printResult(cl_ulong4 seed, cl_ulong round, result r, cl_uchar score, const std::chrono::time_point<std::chrono::steady_clock> & timeStart, const Mode & mode) {
	...
	if (g_bAutoKey) {
		// Compute final private key = seedPrivKey + partialPrivKey mod n
		mp_number partialKey = KeyGen::fromHex(strPrivate);
		mp_number finalKey;
		KeyGen::addModN(finalKey, g_seedPrivKey, partialKey);
		std::cout << " Private: 0x" << KeyGen::toHex(finalKey) << ' ';
	} else {
